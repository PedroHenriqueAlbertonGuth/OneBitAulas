const average = (...numbers) => {
  return `A média simples de ${numbers} é ${(numbers.reduce((valorAcumulado, number) => valorAcumulado + number, 0) / numbers.length).toFixed(2)}`;
};
console.log(average(6, 8, 7, 9, 4, 10));
const weightedAverage = ([...numbers], [...weigth]) => {
  return `A média ponderada dos valores ${numbers} com respectivos pesos ${weigth} é ${numbers.reduce((valorAcumulado, num, i) => valorAcumulado + num * weigth[i], 0) / weigth.reduce((valorAcumulado, num) => valorAcumulado + num, 0)}`;
};
...
const mediana = (...numbers) => {
  ...
  if (numbers.length % 2 === 0) {
    ...
    return `A mediana dos valores ${numbers} é ${mediana.reduce((accum, num) => accum + num, 0) / 2}`;
