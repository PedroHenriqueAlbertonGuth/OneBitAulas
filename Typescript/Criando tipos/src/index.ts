...
type planets = {
  ...
  situacao: "Habitado" | "Habitável" | "Inabitável" | "Inexplorado";
  satelites: string[];
};

function findPlanet(nome: planets["nome"]) {
  return allPlanets.find((e) => e.nome === nome);
}

function savePlanet(planet: planets) {
...
function createPlanet() {
  ...
  const coordenadas: planets["coordenadas"] = [
    Number(prompt(`Qual a coordenada x?`)),
    Number(prompt(`Qual a coordenada y?`)),
  ];
  const situacao: planets["situacao"] = prompt(`Qual a situação do planeta?
    "Habitado" | "Habitável" | "Inabitável" | "Inexplorado"`) as planets["situacao"];
  let satelite;
  let satelites = [];
  do {
...
function removeSatelite(nomePlaneta: planets["nome"], nomeSatelite: string) {
...
function listPlanets() {
  allPlanets.forEach((e) => {
    alert(
      `O Planeta ${e.nome}, de coordenadas ${e.coordenadas} está em situação ${e.situacao} e tem os satelites ${e.satelites}`,
    );
  });
}
  ...
  switch (userOption) {
    ...
    case 2:
      updatePlanetSituation(
        prompt("Qual o nome do planeta?"),
        prompt(
          `Qual a situação do planeta? Habitado" | "Habitável" | "Inabitável" | "Inexplorado`,
        ) as planets["situacao"],
      );
      break;
    case 3:
      ...
      let satelites = [];
      do {
        const satelite = prompt(
          `Escreva o nome do satélite, digite 2 para finalizar`,
        );
        if (satelite !== "2") {
          satelites.push(satelite);
        } else {
    ...
    case 4:
      removeSatelite(
        prompt("Qual o nome do planeta?"),
        prompt("Qual o nome do satélite?"),
      );
      break;
    case 5:
      listPlanets();
